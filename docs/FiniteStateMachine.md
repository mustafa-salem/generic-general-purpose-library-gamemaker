/*******************************************************************************/
#region    ―――――――――――――――――――― OBJECT_EVENTS ――――――――――――――――――――
/*******************************************************************************/

object_events = [
    "Begin Step Event",
    "Step Event",
    "End Step Event",
    "pre_draw_event",
    "draw_begin_event",
    "Draw Event",
    "draw_end_event",
    "post_draw_event",
    "draw_gui_begin_event",
    "draw_gui_event",
    "draw_gui_end_event",
    "game_start_event",
    "game_end_event",
    "Room Start Event",
    "Room End Event",
]

/*******************************************************************************/
#endregion ―――――――――――――――――――― OBJECT_EVENTS ――――――――――――――――――――
/*******************************************************************************/

# Basics
.add(state_name, [state_struct])
->
.change(state_name, [leave_func], [enter_func], [data])
-> change_state
.state_is(state_name, [state_to_check])
-> is_state
.state_exists(state_name)
-> exists_state
.get_states()
-> get_state_names
.get_current_state()
->
.get_previous_state()
->
.get_time([in_microseconds])
-> get_state_runtime_frames
->
.set_time(time, [in_microseconds])
-> set_state_runtime
.on(event, callback, [context])
# Inheritance
.add_child(parent_state_name, state_name, [state_struct])
->
.inherit()
# Events
.event_set_default_function(event, function)
->
.event_get_current_function()
->
.event_exists(event)
-> exists_event / has_event
.enter()
->
.leave()
->
# Triggered Transitions
.add_transition(transition_name, source_state, dest_state, [condition], [leave_func], [enter_func])
.add_wildcard_transition(transition_name, dest_state, [condition], [leave_func], [enter_func])
.add_reflexive_transition(transition_name, source_state, [condition], [leave_func], [enter_func])
-> set_transition
.transition_exists(transition_name, source_state)
-> exists_transition
.trigger(transition_name, [data])
-> trigger_transition

define_default_event -> set_default_event -> add_event
set_event -> add_event
event_exists -> has_event
define_state -> set_state -> add_state
inherit_event -> execute_parent_event
define_transition -> set_transition -> add_transition
set_default_event

state_name
parent_name
event_name
event_method
events
source_state_name
transition_name


change_state_event

// TODO: remove direct references to FiniteStateMachine

/// ----------------------------------------------------------------------------
/// @description
///
/// # States
/// * has_state
/// * get_states
/// * add_state
/// * get_current_state
/// * get_previous_state
/// * change_state
/// *
/// *
/// * get_state_runtime_seconds
/// * add_state_runtime
///
/// # Events
/// * has_event
/// * add_event
/// * execute_parentevent
///
/// # Event Listeners
/// * attach_eventhandler
/// * remove_event_listener
///
/// # Transitions
/// * has_transition
/// * add_transition
/// * trigger_transition
///
/// # History
/// * is_history_enabled
/// * get_history_capacity
/// * get_history_contents
/// * set_history
/// ----------------------------------------------------------------------------