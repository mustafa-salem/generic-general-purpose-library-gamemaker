...
function scr_cutscene_loadstate() {
    ...
    for (i = 0; i < array_length_1d(masterArray); i++) { variable_instance_set(id, masterArray[i], myVar[i]) }
...
function scr_cutscene_commands() {
    ...
    if (_c == "mus" or _c == "music") {
        ...
        if (command_arg1[i] == "play") { /* play -> (OVERWORLD_SOUND_INSTANCE) */ }
        if (command_arg1[i] == "stop") { OVERWORLD_SOUND_INSTANCE.stop() }
        if (command_arg1[i] == "free_all") { gamemaker_sound_stop({ sound : "MUSIC_AUDIO_LABEL" }) }
        if (command_arg1[i] == "free") { OVERWORLD_SOUND_INSTANCE.stop() }
        if (command_arg1[i] == "pause") { OVERWORLD_SOUND_INSTANCE.pause() }
        if (command_arg1[i] == "resume") { OVERWORLD_SOUND_INSTANCE.resume() }
        ...
        if (command_arg1[i] == "volume") { gamemaker_sound_set_gain(OVERWORLD_SOUND_INSTANCE, command_arg2[i], command_arg3[i]) }
        if (command_arg1[i] == "pitch") { gamemaker_sound_set_pitch(OVERWORLD_SOUND_INSTANCE, command_arg2[i]) }
        if (command_arg1[i] == "pitchtime") { gamemaker_sound_set_pitch(OVERWORLD_SOUND_INSTANCE, command_arg2[i], command_arg3[i]) }
