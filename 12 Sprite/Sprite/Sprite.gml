...
function draw_sprite_ext_glow() {
    var colour = (argument_count > 9 ? argument[9] : #FFFFFF)
    var blend = (colour != #FFFFFF ? merge_colour(argument[7], colour, glow) : argument[7])
    if (glow < 1) { draw_sprite_ext(argument[0], argument[1], argument[2], argument[3], argument[4], argument[5], argument[6], blend, argument[8]) }
    if (glow > 0) {
        if (colour == #FFFFFF) { gamemaker_blendmode_set(ADDITIVE_BLENDMODE) }
        ...
        gpu_set_fog(false, #000000, 0, 0)
        if (colour == #FFFFFF) { gamemaker_blendmode_set(DEFAULT_BLENDMODE) }
    }
}
...
function draw_sprite_part_ext_glow() {
    var colour = (argument_count > 12 ? argument[12] : #FFFFFF)
    var blend = (colour != #FFFFFF ? merge_colour(argument[10], colour, glow) : argument[10])
