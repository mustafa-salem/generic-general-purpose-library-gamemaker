...
#macro PRIMITIVE INITIALIZED_CONSTRUCTOR(Primitive)
    ...
    if (struct_exists(arguments, "canvas_scale")) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (p.vertices[i][0] == _xmax) {
                ...
                p.vertices[i][0] += Interpolation.inverseLerp(_xmin, _xmax, p.vertices[i][0]) * (a.canvas_scale - 1)
            }
            // Y
            if (p.vertices[i][1] == _ymax) {
                p.vertices[i][1] += (a.canvas_scale - 1)
            } else if (p.vertices[i][1] != _ymin) {
                p.vertices[i][1] += Interpolation.inverseLerp(_ymin, _ymax, p.vertices[i][1]) * (a.canvas_scale - 1)
            }
        }
        */
    }
