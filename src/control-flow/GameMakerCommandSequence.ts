...
function __gamemaker_commandsequence_add_command(parameters = {}) {
    commands.push(method(parameters.execution_context, parameters.callable))
    return _return
}
...
function scr_cutscene_commands() {
    ...
    if (_c == "var") {
        ...
        } else if (typeof command_arg6[i] != "string") {
    ...
    if (_c == "facing") {
        if (command_arg1[i] == "l" or command_arg1[i] == "d" or command_arg1[i] == "r" or command_arg1[i] == "u") {
...
///
/// # Execution Context
/// When executing command the 'this' context defaults to the instance where the
/// cutscene was originally created and the 'other' context is the cutscene itself.
/// The 'this' context may be set to any other value.
class GameMakerCommandSequence {
    ...
    if (terminate_this_frame == 1) {
        if (kill_actors == 1) {
            for (let jj = 0; jj < 20; jj++) {
                GameObject.destroy({ instance : actor_id[jj] })
            }
        }
        GameObject.destroy(this.gameObject);
    }
