function Menu(_row_count, _column_count, _visible_count, _page_flip_type, _wrap_horizontally = true, _wrap_vertically = true) constructor {
    ...
    *******************************************************************************/
    ...
    page_count = function() { return ceil(entry_count() / page_size()) }
    ...
    row = function(_index) { return (floor(_index / column_count()) % row_count()) }
    ...
    visible_entry_count = function() {  min(max_visible_entry_count(), entry_count() - (page_size() * (page_count() - 1))) }
    ...
    keep_in_range = function(_index) { return min((entry_count() - 1), max(0, _index)) }
    ...
    entries = function() {
        ...
        var _entries = array_create(_entry_count)
        for (var i = 0; i < _entry_count; i++) { _entries[i] = entry(i) }
        return _entries
    }
    ...
    visible_entries = function() {
        ...
        var _visible_entries = array_create(_visible_entry_count)
        for (var i = 0; i < _visible_entry_count; i++) { _visible_entries[i] = visible_entry(i) }
